...
func TestFetchVulns_NoPagination(t *testing.T) {
	...
	result, err := FetchVulns(client, server.URL, "1.20.0")
...
func TestFetchVulns_EmptyResponse(t *testing.T) {
	...
	client := retryablehttp.NewClient()
	client.HTTPClient = server.Client()
	result, err := FetchVulns(client, server.URL, "99.99.99")
	require.NoError(t, err)
...
func TestFetchVulns_LargePagination(t *testing.T) {
	...
	client := retryablehttp.NewClient()
	client.HTTPClient = server.Client()
	result, err := FetchVulns(client, server.URL, "1.19.0")
	require.NoError(t, err)
...
// TestCPEStringFormat verifies the CPE string is correctly formatted for Gitea
func TestCPEStringFormat(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "gitea version 1.20.0",
			version:  "1.20.0",
			expected: "cpe:2.3:a:gitea:gitea:1.20.0:*:*:*:*:*:*:*",
		},
		{
			name:     "gitea version 1.19.3",
			version:  "1.19.3",
			expected: "cpe:2.3:a:gitea:gitea:1.19.3:*:*:*:*:*:*:*",
		},
	}
