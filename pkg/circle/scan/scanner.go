...
func InitializeOptions(input InitializeOptionsInput) (ScanOptions, error) {
	...
	httpClient := httpclient.GetPipeleekHTTPClient("", nil, nil).StandardClient()
	...
	if len(projects) == 0 {
		if strings.TrimSpace(input.Organization) != "" {
			...
			if err != nil {
				...
				if fallbackErr != nil {
					hint := orgDiscoveryHint(input.Organization)
					if hint != "" {
						return ScanOptions{}, fmt.Errorf("ListOrganizationProjects failed: %v; fallback ListAccessibleProjectsV1 failed: %w. Hint: %s", err, fallbackErr, hint)
					}
