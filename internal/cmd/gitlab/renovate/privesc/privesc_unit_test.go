...
func TestGLNewPrivescCmd(t *testing.T) {
	cmd := NewPrivescCmd()
	assert.NotNil(t, cmd)
	assert.Equal(t, "privesc", cmd.Name())
	assert.Contains(t, cmd.Short, "malicious CI/CD Job")
}

func TestGLPrivescCmdFlags(t *testing.T) {
	cmd := NewPrivescCmd()
...
func TestGLPrivescCmdMonitoringIntervalFlagDefaults(t *testing.T) {
	cmd := NewPrivescCmd()
