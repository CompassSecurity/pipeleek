...
var flagBindings = map[string]string{
	...
	"project":                 "gitlab.renovate.privesc.project",
...
func NewPrivescCmd() *cobra.Command {
	privescCmd := &cobra.Command{
		...
		Example: `pipeleek gl renovate privesc --token glpat-xxxxxxxxxxx --url https://gitlab.mydomain.com --project mygroup/myproject --renovate-branches-regex 'renovate/.*'`,
		Run: func(cmd *cobra.Command, args []string) {
			if err := config.AutoBindFlags(cmd, flagBindings); err != nil {
				log.Fatal().Err(err).Msg("Failed to bind command flags to configuration keys")
			}

			if err := config.RequireConfigKeys("gitlab.url", "gitlab.token", "gitlab.renovate.privesc.project"); err != nil {
				log.Fatal().Err(err).Msg("required configuration missing")
			}

			privescRenovateBranchesRegex = config.GetString("gitlab.renovate.privesc.renovate_branches_regex")
			privescProject = config.GetString("gitlab.renovate.privesc.project")
	...
	privescCmd.Flags().StringVarP(&privescProject, "project", "p", "", "The project to target (format: group/project)")
